...
logger = logging.getLogger("1099_nec_extractor")
...
# ---------------------------------------------------------------------------
# JSON Schema (1099-NEC)
# ---------------------------------------------------------------------------

FORM_1099_NEC_OUTPUT_SCHEMA = {
    "$schema": "http://json-schema.org/draft-07/schema#",
    "title": "Standardized_1099_NEC_Schema",
    "type": "object",
    "properties": {
        "document_type": { "type": "string", "const": "1099" },
        "subtype": { "type": "string", "const": "1099-NEC" },
        ...
        "form_metadata": {
            "type": "object",
            "properties": {
                "form_id": { "type": "string", "const": "1099-NEC" },
        ...
        "tax_data_boxes": {
            ...
            "properties": {
                "nonemployee_compensation": { "type": ["number", "N/A"] },
                "excess_golden_parachute_payments": { "type": ["number", "N/A"] },
                ...
                "state_income": { "state_income_1": {"oneOf": [{"type": "number"},{"const": "N/A"}]}, 
                    
                    "state_income_2": {"oneOf": [{"type": "number"},{"const": "N/A"}]},  },
                    
                "state_payers_state_no": { "state_payers_state_no_1": {"oneOf": [{"type": "string"},{"const": "N/A"}]}, 
                    
                    "state_payers_state_no_2": {"oneOf": [{"type": "string"},{"const": "N/A"}]},  
                }
            }
        }
    },
...
You are processing exclusively Form 1099-NEC (Nonemployee Compensation).

### Operational Workflow:
1. CONFIRM FORM SUB-TYPE: Verify the document is a 1099-NEC by identifying the title cluster ["Nonemployee", "Compensation"] in the header of the document. Set `subtype` to "1099-NEC" and `form_metadata.form_id` to "1099-NEC". Keep the root `document_type` strictly set to "1099".
2. SELECT FIELD SCOPE: Extract data *only* for the keys defined under "tax_data_boxes" for 1099-NEC. Do not include fields from any other 1099 sub-type.
...
2. BOX IDENTIFICATION & ALIGNMENT: Match currency values to their respective form box labels for 1099-NEC:
   - Box 1: Nonemployee Compensation
   - Box 2: Direct Sales totaling $5,000 or more (checkbox/boolean)
...
5. CLEAN NULL HANDLING:
   - If a field belongs to the 1099-NEC form but contains no value in the raw OCR text, explicitly output it as `N/A`.
...
2. AMBIGUOUS WORD ISOLATION (OVERLAP PREVENTION):
   The word "Compensation" will appear as BOTH the form title and a tax box label. Differentiate by context:
   ...
   - Tax Box Label: Strictly anchored alongside a numeric box sequence (e.g., "Box 1 Nonemployee Compensation").
...
def build_extraction_prompt(ocr_text: str) -> str:
    schema_str = json.dumps(FORM_1099_NEC_OUTPUT_SCHEMA, indent=2)
    return f"""
Below is the OCR-extracted text from a Form 1099-NEC document:

--- OCR TEXT START -----
{ocr_text}
--- OCR TEXT END -----

Extract all relevant 1099-NEC tax information and return a JSON object that strictly 
follows this schema:
...
def parse_llm_response(raw_text: str) -> dict:
    ...
    try:
        ...
        return validate_1099_nec_schema(parsed)
    except json.JSONDecodeError as e:
        logger.error("JSON parsing failed: %s", e)
        raise ValueError(f"LLM returned invalid JSON: {e}") from e

def extract_1099_nec_with_bedrock(md_text: str) -> dict:
    logger.info("Sending text payload to Bedrock for structured 1099-NEC extraction...")
...
# ---------------------------------------------------------------------------
# Data Validation & Normalization Logic (1099-NEC)
# ---------------------------------------------------------------------------
...
_MONETARY_FIELDS={
    ...
    # "direct_sales_totaling_5000_or_more",
    "federal_income_tax_withheld",
}



def validate_1099_nec_schema(data: dict) -> dict:
    """
    Validate and normalize extracted Form 1099-NEC data variables.
    """
    ...
    state_withheld_data = tax_boxes.get("state_tax_withheld") or 0.0
    ...
    logger.info("1099-NEC schema validation complete. Status: %s", validation.get("validation_status"))
    return data
...
def extract_1099_nec(
    file_path: str,
    use_textract: bool = True,
) -> dict:
    logger.info("=" * 60)
    logger.info("Form 1099-NEC Extraction Pipeline Started")
    ...
    if use_textract:
        try:
            ...
            if ocr_text.strip():
                form_data = extract_1099_nec_with_bedrock(ocr_text)
            else:
...
def save_results(data: dict, output_path: str = "1099nec_extracted.json") -> None:
...
#     import sys
#     document_path = sys.argv[1] if len(sys.argv) > 1 else "1099_NEC.pdf"

#     try:
#         result = extract_1099_nec(
#             file_path=document_path,
#             use_textract=True,
#         )
#         save_results(result, output_path="json/1099nec_extracted_output.json")
