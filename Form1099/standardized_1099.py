...
def extract_text_with_textract(file_path: str) -> str:
    ...
    # print(response.get("Blocks", []))

    # block = response.get("Blocks", [])
    # with open("xyz.json", "w", encoding="utf-8") as f:
    ...
    for block in response.get("Blocks", []):
        if block.get("BlockType") == "LINE" and block.get("Text") == "Form 1099-NEC":
            print(block)
            # print("Text       :", block.get("Text"))
            # print("Confidence :", block.get("Confidence"))
...
def validate_1099_schema(data: dict) -> dict:
    """
    Validate and normalize extracted Form 1099 data variables.
    """
