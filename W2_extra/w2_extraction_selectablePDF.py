# # pdf_path = "C:/Users/Lenovo/Downloads/fw2.pdf"

# import fitz

# doc = fitz.open("C:/Users/Lenovo/Downloads/fw2.pdf")  # example document
...
# ---------------------------------------------------------------------------
# Step 1: OCR Extraction via Amazon Textract
# ---------------------------------------------------------------------------
def extract_text_with_textract(file_path: str) -> str:
    ...
    # # extracted_text = "\n".join(lines)
    extracted_text = str(to_markdown(file_path))
    # logger.info(
...
def extract_w2_with_bedrock_text(ocr_text: str) -> dict:
    """
    Send OCR text to AWS Bedrock Claude for structured W-2 extraction.
...
def pretty_print_summary(w2_data: dict) -> None:
    ...
    print("-" * 50)
    print(f"  Wages           : ${wages.get('wages_tips_other_compensation', 0):,.2f}")
    print(f"  Federal Tax     : ${wages.get('federal_income_tax_withheld') or 0:,.2f}")
    print(f"  SS Wages        : ${wages.get('social_security_wages', 0):,.2f}")
    print(f"  SS Tax          : ${wages.get('social_security_tax_withheld', 0):,.2f}")
    print(f"  Medicare Wages  : ${wages.get('medicare_wages_and_tips', 0):,.2f}")
    print(f"  Medicare Tax    : ${wages.get('medicare_tax_withheld', 0):,.2f}")
    print("-" * 50)
...
if __name__ == "__main__":
    ...
    try:
        ...
        save_results(result, output_path="pymupdf4llm_extraction_using_markdown.json")
